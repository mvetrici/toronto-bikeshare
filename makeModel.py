...
# in progress
def dura_dist(df: pd.DataFrame, groupby: str = ''):
    ...
    if groupby and groupby not in df.columns:
        add_col_Periods(df, [groupby]) # doesn't modify df
...
def DEADtrips_per_hour(df: pd.DataFrame):
...
class makeModel():
    ...
    def __init__(self, df: pd.DataFrame, predict: str, variables: list[str]):
        ...
        # # FIX ERROR!!!
        self.df = df.copy()
        self.predict = get_label(df.columns, predict)
        self.vars = []
        for var in variables:
            var = get_label(df.columns, var)
    ...
    def make_dot(self, x: str, y: str): 
        x = get_label(self.df.columns, x)
        y = get_label(self.df.columns, y)
        self.df.plot(x = x, y = y, kind = 'scatter')
        plt.show()
    
    def make_lm(self, group: str):
        grouping = get_label(self.df.columns, group)
